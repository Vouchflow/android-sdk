...
#       Optionally, before starting a new Gradle process, a hook can be
#       configured to let the caller set up environment variables, control JVM
#       options, and other aspects by defining the function `beforeGradle`
#       in the script. The full description of the function can be found in
...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        ...
        # temporary fixes as there are arguments to shift them in
        set -- "$@" "$arg"
        shift
    done
fi
